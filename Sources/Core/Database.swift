...
final class Database {
    ...
    init(path: String) throws {
        ...
        if sqlite3_open_v2(path, &handle, SQLITE_OPEN_CREATE | SQLITE_OPEN_READWRITE | SQLITE_OPEN_FULLMUTEX, nil) != SQLITE_OK {
            let message = String(cString: sqlite3_errmsg(handle) ?? "Unknown error")
    ...
    func query(_ sql: String, with bindings: [SQLiteValue] = []) throws -> [[String: SQLiteValue]] {
        let stmt = try prepare(sql)
        defer { sqlite3_finalize(stmt) }
        try bind(bindings, to: stmt)
    ...
    private var lastErrorMessage: String {
        String(cString: sqlite3_errmsg(db) ?? "Unknown SQLite error")
    }
}
...
extension Database {
    static func defaultPath(fileName: String = "app.sqlite3") throws -> String {
