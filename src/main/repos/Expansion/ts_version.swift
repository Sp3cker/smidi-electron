...
struct TSVersion {
      ...
      func parseLineFast(_ line: Substring, symbols: TSSymbolMaps) -> TSNode? {
            ...
            let macro = line[i..<space]
            let argsStr = line[line.index(after: space)...].trimmingCharacters(in: .whitespaces)
            let rawArgs: [String] = argsStr.split(separator: ",").map { String($0.trimmingCharacters(in: .whitespaces)) }
            ...
            if macro == "voice_keysplit" || macro == "voice_keysplit_all" {
                  ...
                  return TSNode(type: .keysplit, voicegroup: first, macro: String(macro), params: rawArgs, sampleSymbol: nil, assetPath: nil, samples: nil)
            }
            if macro.hasPrefix("voice_directsound") {
                  let sample: String = rawArgs.count > 2 ? rawArgs[2] : ""
                  let asset = symbols.directSound[sample]
                  return TSNode(type: .directsound, voicegroup: nil, macro: String(macro), params: rawArgs, sampleSymbol: sample, assetPath: asset, samples: nil)
            }
            if macro.hasPrefix("voice_programmable_wave") {
                  let sample: String = rawArgs.count > 2 ? rawArgs[2] : ""
                  let asset = symbols.programmable[sample]
                  return TSNode(type: .programmable, voicegroup: nil, macro: String(macro), params: rawArgs, sampleSymbol: sample, assetPath: asset, samples: nil)
            }
            if macro.hasPrefix("voice_square") { return TSNode(type: .square, voicegroup: nil, macro: String(macro), params: rawArgs, sampleSymbol: nil, assetPath: nil, samples: nil) }
            if macro.hasPrefix("voice_noise") { return TSNode(type: .noise, voicegroup: nil, macro: String(macro), params: rawArgs, sampleSymbol: nil, assetPath: nil, samples: nil) }
            return TSNode(type: .unknown, voicegroup: nil, macro: String(macro), params: rawArgs, sampleSymbol: nil, assetPath: nil, samples: nil)
      }
      ...
      func resolveGroup(label: String, symbols: TSSymbolMaps, cache: inout [String: TSNode], stack: inout Set<String>) throws -> TSNode {
            if let cached = cache[label] { return cached }
            if stack.contains(label) {
                  return TSNode(type: .group, voicegroup: label, macro: nil, params: nil, sampleSymbol: nil, assetPath: nil, samples: [TSNode(type: .unknown, voicegroup: nil, macro: nil, params: ["cycle detected"], sampleSymbol: nil, assetPath: nil, samples: nil)])
            }
            stack.insert(label)
            ...
            for node in entries {
                  if node.type == .keysplit, let sub = node.voicegroup {
                        ...
                        let subnode = try resolveGroup(label: sub, symbols: symbols, cache: &cache, stack: &localStack)
                        let merged = TSNode(type: .keysplit, voicegroup: node.voicegroup, macro: node.macro, params: node.params, sampleSymbol: nil, assetPath: nil, samples: subnode.samples)
                        children.append(merged)
                  } else {
                        children.append(node)
                  }
            }
            let group = TSNode(type: .group, voicegroup: label, macro: nil, params: nil, sampleSymbol: nil, assetPath: nil, samples: children)
            cache[label] = group
            stack.remove(label)
            return group
      }
      ...
      func writeJSON(label: String, node: TSNode) throws {
            ...
            let url = URL(fileURLWithPath: FileManager.default.currentDirectoryPath).appendingPathComponent("\(label).json")
...
struct TSMain {
      static func run(label: String = "voicegroup229") {
            ...
            do {
                  print("YOO")
                  let start = Date()
                  let root = try ts.resolveGroup(label: label, symbols: symbols, cache: &cache, stack: &stack)
