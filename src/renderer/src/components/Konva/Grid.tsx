...
/**
 * Beat-based grid component for finer granularity within measures
 */
export const BeatGrid = ({
  ...
  const beatLines = useMemo(() => {
    ...
    return Array.from({ length: numBeats + 1 }, (_, i) => {
      const x = i * pixelsPerBeat;
      const isMeasureBeat = i % BEATS_PER_MEASURE === 0;

      return (
        <Line
          points={[x, 0, x, height]}
          stroke={getCssVariable("--color-neir")}
          strokeWidth={isMeasureBeat ? 2 : 1}
          opacity={isMeasureBeat ? 1 : 0.5}
          key={`beat-${i}`}
        />
      );
    });
  }, [totalBeats, pixelsPerBeat, height]);

  const beatLabels = useMemo(() => {
    if (!showBeatLabels) return null;

    const numBeats = Math.floor(totalBeats);
    return Array.from({ length: numBeats }, (_, i) => {
      const x = i * pixelsPerBeat + 2;
      const beatInMeasure = (i % BEATS_PER_MEASURE) + 1;
  ...
  }, [totalBeats, pixelsPerBeat, showBeatLabels]);
